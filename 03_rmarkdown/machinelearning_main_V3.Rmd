...
```
As the output shows the additionally added variable of sex with a calculated p-value of 0.0149 seems to have an effect on charges and therefore should be considerd within the modelling process.

#### Including interaction  

Since a second variable was added, we want to explore if there might be significant interactions between age and sex, that might have to be considered for the modeling process. 
```{r}
...
```
The R-output shows that there isn't any significant interaction between age and sex that has to be considerd in our model. 
...
Our residual mean deviance (R-squared-error) us at 0.1592. Note that this value is also log transformed as the numbers on which it has been calculated are log transformed.


#### Prediction

We are now again predicting the data that has already been used for training the modell.
...
We  can furthermore clearly see that the residuals are roughly normalized, with an approximate mean of 0. Outliers are however more distributed in the negative axis of y than in the postiive.
...
```{r}
hist(error_regression.charges1, main = "Histrogram of the Error for charges")
```
...
We now receive a tree with exactly the same number of nodes but where the criterias have different values, except for the first node, which remain the same (non-smoker --> left branch, smoker --> right branch).
...
We here receive a classification tree with four end nodes. Furthermore our RMS is at 2.604 and the misclassification rate - which is basically the RSS for classification trees - is at at 65.47%.
...
Whereas the model performed a bit better with the train data in comparison with the entire data set, the model now clearly performs in an all time bad. The training modell is thus a clear overfit.
...
What is intersting to see is that between the number of end nodes 4 and 7, there is a slight growth as well as decline in terms of variance, so the slope inbetween is both negative as well as positive.
...
By setting FUN to prune.misclass, we communicate that this decision tree will be handling a continous variable.
...
We also compute the mean: in comparison with the previously caluclated mean value, we can say they are rather related than not, however with some deviance.
...
We here see that age and if one smokes are of biggest influence on charges. Age also seems fairly imporant.
...
summary(lm.insurance.3)$r.squared
summary(lm.insurance.all)$r.squared
summary(tree.regression.charges)$r.squared
summary(rf.insurance)$r.squared
...
for(i in 1:100){
  ...
  #RMSE.3[i] <- RMSE(tree.regression.charges.predict, insurance.test$charges)
  #RMSE.4[i] <- RMSE(rf.insurance.predict, insurance.test$charges)
...
boxplot(RMSE.4, main="random forest")
```
