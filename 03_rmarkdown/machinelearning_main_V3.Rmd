...
One can see that BMI seems to be normally distributed whereas charges follow a 1/x-function. 
...
We here receive a tree with six terminal nodes. For variables to determine the BMI, "charges" have been applied as well as if one smokes and from which region noe comes. This can also be seen in below summary:
...
By working with the test data, we already can see at this point a change. Were there previously only six end nodes, we now have seven. 
...
We receive a tree with four end nodes and three influencing variables, as also the summary further demonstrates:
...
We  can furthermore clearly see that the variable is not distributed according to the normal distribution. 
...
cat(tree.classification.region.pred.test)
...
ratio <- 0.6
total <- nrow(insurance)
train.region <- sample(1:total, as.integer(total * ratio))
tree.regression.region2 <- tree(region~., insurance, subset=train)
plot(tree.regression.region2)
text(tree.regression.region2, pretty=12, cex=0.75)
```

```{r}
tree.classification.region.pred.ct2 <- table(tree.classification.region2, insurance[train,]$region)
tree.classification.region.pred.correct2 <- 0
tree.classification.region.pred.error2 <- 0
for (i1 in 1:4) {
  for (i2 in 1:4) {
    if (i1 == i2) {
      tree.classification.region.pred.correct2 <- tree.classification.region.pred.correct2 + tree.classification.region.pred.ct2[i1,i2]
    }else{
      tree.classification.region.pred.error2 <- tree.classification.region.pred.error2 + tree.classification.region.pred.ct2[i1,i2]
    }
  }
}
tree.classification.region.pred.rate2 <- tree.classification.region.pred.correct2/sum(tree.classification.region.pred.ct2)
tree.classification.region.pred.error2 <- 1 - tree.classification.region.pred.rate2
cat(tree.classification.region.pred.rate2)
cat("\n")
cat(tree.classification.region.pred.error)
```
