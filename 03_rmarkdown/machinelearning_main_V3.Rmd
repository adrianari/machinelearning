...
We here receive a tree with six terminal nodes. Thanks to the tree-function, the tree is created by recursive binary splitting.
The variables (internal nodes) determining the BMI, "charges" have been applied as well as if one smokes and from which region one comes. This can also be seen in below summary:
...
Whereas the model performed a bit better with the train data in comparison with the entire data set, the model now clearly performs in an all time bad. The train modell is thus a clear overfit.
