...
ggplot(data = insurance,
         mapping = aes(y = charges,
                       x = sex)) +
    geom_boxplot() + ggtitle("charges depending on gender")
```
...
In the above table you can fint the latest results for all created models which are nearly the same.
...
We now recheck if misclassification improves when cross-validiting our model:
