...
As usual, we will start by fitting a simple regression model to the insurance dataset. Therefore we will start with one variable and add additonal complexity in each following subset. 
...
As shown in the R Output a person with the age of 0 will have a base charge of 3165.885. This intercept and its interpreation seems to be nonsensical, thus one needs to pay attention in what extend this model can be applied.

With each year, charges increase by 257.7226, which is the slope for the regressionline.
...
```
As the output shows the additionally added variable of sex with a calculated p-value of 0.0149 seems to have an effect on charges and therefore should be considerd whithin the modeling process.
...
```
As the above R Output shows, not every variable seems to have a significant effect on the charges. Therfore we will seperatly test the varaibles of sex and region before dropping them from our model.
...
```
Using the visualization it seems that there seems to be no obvoius difference and effect. Therefore, we decide to drop also the variable region from our linear model.
...
```{r}
set.seed(7)
r.squared.lm.insurance.3 <- c()
r.squared.lm.insurance.all <- c()
r.squared.tree.regression.charges <- c()
r.squared.rf.insurance <- c()
...
for(i in 1:100){
  ...
  RSS_charges <- sum((insurance$charges-tree.regression.charges.predict^2)
  MSE_charges <- RSS_charges/length(tree.regression.charges.predict) 
  deviation_charges <- sqrt(MSE_charges)
...
mean(RMSE.1)
#lm.amm
mean(r.squared.lm.insurance.all)
#tree regression
mean(r.squared.tree.regression.charges)
#random forest
mean(r.squared.rf.insurance)
```

```{r}
par(mfrow=c(1,4))
boxplot(r.squared.lm.insurance.3, main="lm.3")
boxplot(r.squared.lm.insurance.all, main="lm.all")
boxplot(r.squared.tree.regression.charges, main="regression tree")
boxplot(r.squared.rf.insurance, main="random forest")
```
