...
There might be an affect between the number of children and charges. As shown above, it might be possible to interpret that with a rising number of children the charges are decreasing. Still, this is not a clear relationship, more a wide interpretation of the plot.
...
In this part we want to show exemplary how the relationship between charges and age can be affected by adding additionally a categorical variable. This can be used for further data explorations and can be adapted on all other variables. For this section, we will just perform this enlargement for the case of the age to illustrate some possible relationships, which also can be helpful for the following modeling process.
```{r}
...
As usually we will start by fitting a simple regression model to the insurance dataset. Therefore we will start with one variable and add additonal complexity in each following subset. 
...
With each year the charges increase by 257.7226, which is the slope for this regressionline

#### P-values  

With a value of 2e-16, age seems to have a very strong effect on charges. This means that the slope of the chages is not flat, not zero, so the hypothesis has to be thrown away.
