...
By working with the test data, we already can see at this point a change, meaning a possible overfit of working with our entire dataset. Were there previously six end nodes, we now have seven end notes, with variables used "charges", "smoker", "region" and "age", as also can be seen here:

```{r}
summary(tree.regression.bmi2)
```

Our residual mean deviance has now gone from 29.23 to 27.84.
...
We receive a RSS for the train dataset of 27.21 whereas for the test set, the RSS is 34.55. We seem to have an overfit of the model to the train data as the error rate for the test set is considerable higher than the true error (29.23)
...
Both dataframes show the about same distribution as can be seen in the boxplot. 
...
Our residual mean deviance (R-squared-error) us at 0.1592. Note that this value is also log transformed just like the other numbers regarding distribution.
...
The boxplot clearly shows that there are more outliers in the negative area of the y-axis.

```{r}
hist(error_regression.charges1)
```

Here goes to further show that the data points are slightly skewed.
...
Our residual mean deivane is now a bit lower at 0.1475 (previously: 0.1592)
...
We here receive a classification tree with four end nodes. Furthermore our RMS is at 2.604 and the misclassification rate is at at 65.47%.
...
We now examine the classification error on the train data.
...
We here see that the tree with seven end nodes has the smallest cross-validation error of about 27'250. This is also further shown in below table.

```{r}
tree.regression.bmi.pruning
```

What is intersting to see is that with a tree size of seven, the error rate slight rises but then falls again for a tree with 4 end nodes just to then dramatically rise again.
