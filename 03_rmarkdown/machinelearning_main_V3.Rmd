...
With a value of 2e-16, age seems to have a very strong effect on charges. This means that the slope of charges is not flat, not zero, so the hypothesis has to be thrown away. 
...
```
As the above R Output shows, not every variable seems to have a significant effect on the charges.

#### Testing sex before dropping 

Before we remove those two variables we first will make a deeper analysis.
```{r}
